...
from md3d.builders.config_builder_util import ConfigObj
from md3d.core import box_3d_encoder

from md3d.datasets.kitti import calib_utils, depth_map_utils, instance_utils
...
class ObjectFilter:
    ...
    @staticmethod
    def create_obj_filter(obj_types,
                          difficulty,
                          occlusion,
                          truncation,
                          box_2d_height,
...
class ObjectLabel:
    ...
    def __init__(self):
        self.obj_type = None  # Type of object
        self.truncation = -1.0
        self.occlusion = -1
        self.alpha = 0.0
        self.x1 = 0.0
    ...
    def __repr__(self):
        ...
        return '({}, a:{}, [{:0.2f} {:0.2f} {:0.2f}], ' \
               '({:.2f}, {:.2f}, {:.2f}), {:.2f}, {:.3f})'.format(
                   self.obj_type, self.alpha, self.t[0], self.t[1], self.t[2],
                   self.l, self.w, self.h, self.ry, self.score)

    def box_2d(self):
        return np.asarray([self.y1, self.x1, self.y2, self.x2])

    def box_2d_iou_fmt(self):
        return np.asarray([self.x1, self.y1, self.x2, self.y2])

    def box_3d(self):
        return np.asarray([self.t[0], self.t[1], self.t[2], self.l, self.w, self.h, self.ry])
...
def filter_labels_by_obj_type(obj_labels, classes):
    ...
    """
    obj_type_mask = [(obj.obj_type in classes) for obj in obj_labels]
    return obj_labels[obj_type_mask], obj_type_mask
...
def filter_labels_by_score(obj_labels, score):
    """Filters object labels within a range of depth values.

    Args:
        obj_labels: List of object labels
        score: Range of depth to keep objects

    Returns:
        obj_labels: List of filtered labels
        score_mask: Mask of labels to keep
    """

    score_mask = [obj_label.score >= score
                  for obj_label in obj_labels]

    return obj_labels[score_mask], score_mask


def filter_labels(obj_labels, classes=None, difficulty=None,
                  box_2d_height=None, occlusion=None, truncation=None,
                  depth_range=None, score=None):
    ...
    if classes is not None:
        _, class_mask = filter_labels_by_obj_type(obj_labels, classes)
        obj_mask &= class_mask
    ...
    if score is not None:
        _, score_mask = filter_labels_by_score(obj_labels, score)
        obj_mask &= score_mask
...
def obj_classes_from_obj_labels(obj_labels):
    return np.asarray([obj_label.obj_type
                       for obj_label in obj_labels])
...
def get_lidar_point_cloud_for_cam(sample_name, frame_calib, velo_dir,
                                  image_shape=None, cam_idx=2, intensities=False):
    ...
    point_cloud, intensities = get_lidar_point_cloud(
        sample_name, frame_calib, velo_dir, intensities)
    ...
    if image_shape is None:
        ...
        return filtered_point_cloud, intensities[image_filter]
...
def get_depth_map_point_cloud(sample_name, cam_p, depth_dir):
...
def get_ground_plane(sample_name, planes_dir):
...
def compute_box_3d_corners(box_3d):
    ...
    rot = np.asarray([[+np.cos(ry), 0, +np.sin(ry)],
                    [0, 1, 0],
...
def obj_label_to_kitti_fmt(obj):
    ...
    return [
        obj.obj_type, np.round(obj.truncation, 2), np.int32(obj.occlusion),
        np.round(obj.alpha, 3),
        obj.x1, obj.y1, obj.x2, obj.y2,
        np.round(obj.h, 2), np.round(obj.w, 2), np.round(obj.l, 2),
        np.round(obj.t[0], 3), np.round(obj.t[1], 3), np.round(obj.t[2], 3),
        np.round(obj.ry, 3), np.round(obj.score, 3)
    ]


def objs_to_kitti_fmt(obj_labels):
    return [obj_label_to_kitti_fmt(det_obj) for det_obj in obj_labels]


def save_objs_kitti_fmt(out_path, objs_kitti_fmt):
    np.savetxt(out_path, objs_kitti_fmt, newline='\r\n', fmt='%s')
