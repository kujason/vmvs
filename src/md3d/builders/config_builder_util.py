...
def get_configs_from_pipeline_file(pipeline_config_path,
    ...
    output_root_dir = md3d.root_dir() + '/data/outputs/' + checkpoint_name

    if stagewise_training:
        # We are going to train both models one at a time,
        # so store them under the same directory
        model_name = model_config.model_name
        if model_name == 'rpn_model':
            output_root_dir += '/stage1_rpn'
        elif model_name == 'avod_model':
            output_root_dir += '/stage2_avod'
        else:
    ...
    if is_training:
        ...
        if stagewise_training:
            experiment_config_path += '_' + model_config.model_name
